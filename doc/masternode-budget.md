...
PAXCHANGE now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<paxchange_address\> \<monthly_payment_paxchange\> [use_ix(true|false)]
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<paxchange_address\> \<monthly_payment_paxchange\> \<collateral_hash\>
 ...
 * vote-many          - Vote on a PAXCHANGE initiative
 * vote-alias         - Vote on a PAXCHANGE initiative
 * vote               - Vote on a PAXCHANGE initiative/budget
