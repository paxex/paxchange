...
{
    // return if URI is not valid or is no paxchange: URI
...
{
    // Convert paxchange:// to paxchange:
    //
    //    Cannot handle this later, because paxchange:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open paxchange.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PAXEXcoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for PAXEXcoin.lnk
...
{
    return GetAutostartDir() / "paxchange.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a paxchange.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=PAXEXcoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the paxchange app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add paxchange app to startup item list
